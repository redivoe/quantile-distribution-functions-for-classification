...
kfold_results <- function(split,
  ...
  y_train <- y[-test_ids]
  y_test <-  y[test_ids]
  X_train <- as.data.frame(X[-test_ids, ])
  X_test <-  as.data.frame(X[test_ids, ])
  ...
  if("lda" %in% models){
    ...
    out_train[["lda"]] <- predict(object = out_lda)$class
  ...
  if("nb_fgld" %in% models){
    ...
    out_test[["nb_fgld"]] <- predict_naive_bayes(out_nb_fgld, X = X_test)$class
  }
